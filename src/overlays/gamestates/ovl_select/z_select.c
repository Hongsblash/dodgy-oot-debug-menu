...
void MapSelect_UpdateMenu(MapSelectState* this) {
    ...
    // Assuming visibleItemCount is the number of items you can display without scrolling.
    s32 visibleItemCount = (this->sceneTotal > 20) ? 20 : this->sceneTotal;

    // Determine the need for scrolling based on the accumulator
    if (this->verticalInputAccumulator < -7) {
        this->verticalInput = 0;
        this->verticalInputAccumulator = 0;

        // Move selection down
        if (this->currentScene < this->sceneTotal - 1) {
            this->currentScene++;
        }

        // Adjust the topDisplayedScene only if the currentScene goes beyond the visible range
        if (this->currentScene >= this->topDisplayedScene + visibleItemCount) {
            this->topDisplayedScene = this->currentScene - visibleItemCount + 1;
        }
    } else if (this->verticalInputAccumulator > 7) {
        this->verticalInput = 0;
        this->verticalInputAccumulator = 0;
        ...
        // Adjust the topDisplayedScene to ensure the currentScene is visible
        if (this->currentScene < this->topDisplayedScene) {
            this->topDisplayedScene = this->currentScene;
        }
    }

    // Additional logic to ensure `topDisplayedScene` and `currentScene` are kept within valid bounds
    // This is crucial if other parts of your code might adjust these values outside the scrolling logic
    if (this->topDisplayedScene < 0) {
        this->topDisplayedScene = 0;
    }
    if (this->currentScene < 0) {
        this->currentScene = 0;
    }
    if (this->currentScene >= this->sceneTotal) {
        this->currentScene = this->sceneTotal - 1;
    }
    if (this->topDisplayedScene > this->currentScene) {
        this->topDisplayedScene = this->currentScene;
    }

    // Ensure `topDisplayedScene` does not scroll off the end of the list
    if (this->topDisplayedScene > this->sceneTotal - visibleItemCount) {
        this->topDisplayedScene = this->sceneTotal - visibleItemCount;
        if (this->topDisplayedScene < 0) {
            this->topDisplayedScene = 0; // In case sceneTotal is less than visibleItemCount
        }
    }
...
void MapSelect_PrintMenu(MapSelectState* this, GfxPrint* printer) {
    ...
    // Calculate the maximum number of items to display, based on sceneTotal and a visual limit if desired
    s32 displayLimit = (this->sceneTotal > 20) ? 20 : this->sceneTotal;
    
    for (i = 0; i < displayLimit; i++) {
        scene = this->topDisplayedScene + i;
        
        // Break the loop if the scene index exceeds the total number of scenes
        if (scene >= this->sceneTotal) {
            break;
        }
        ...
        // Print the scene name with or without index as needed
        if (scene == 0) { // Adjust if your title screen or first item logic differs
            GfxPrint_Printf(printer, "%s", name);
        } else {
            GfxPrint_Printf(printer, "%03d: %s", scene, name);
        }

        // Mark the current scene with a cursor
        if (scene == this->currentScene) {
